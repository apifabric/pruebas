...
resourcename="pruebas"  # lower case, only
resourcegroup="pruebas_rg"
...
# git clone https://github.com/apilogicserver/pruebas.git
# cd pruebas
...
az group create --name pruebas_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group pruebas_rg --sku S1 --is-linux
az container create --resource-group pruebas_rg --name pruebas --image apilogicserver/pruebas:latest --dns-name-label pruebas --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//pruebas.westus.azurecontainer.io:5656'
